...
In this paper, we focus primarily on HTTP adaptive video. Thus, we chose TCP Reno, as it reported to be the congestion control algorithm used by Netflix \cite{Mishra-2019-the-great-internet-tcp-congestion-control-census}. Additionally, Netflix is reported to have been the most impactful company in terms of network bandwidth with it accounting for over 12.6\% of the Internet traffic overall \cite{Sandvine-2019-global-internet-report}.


TCP Reno connections as implemented in the Linux Kernel 5.4 series reset their slow-start threshold if an idle period longer than the RTO occurs. In contrast, As discussed in Section \ref{sec:background} New Congestion Window Validation sustains its slow start threshold during such periods. This means that the former can resume transferring at its previous sending rate quicker. Thus, New Congestion window validation assumes and is optimised for link stability.
...
Our hypothesis is that New Congestion Window validated connections would help client to obtain stabler capacity estimations which could result in more consistent representation choices.

\section{Experimental Setup}

\todo{Move to section 3. 1st paragraph. To evaluate our Hypothesis - section 4 first sentence}
...
We found that New CWV connections have a lower value for the estimated throughput, compared to Reno connections without newcwv. \todo{More on that when I add the distribution plots}
Additionally, in the tested network scenarios, in terms of video QoE, similar performance was observed for TCP Reno regardless of whether New Cognestion Window Validation was enabled. Therefore, in these cases New CWV makes no difference w.r.t. the QoE or video smoothness. Following are details about the results.

Figures \ref{fig:throughput-precise-DSL} - \ref{fig:throughput-precise-FTTP} show the throughput distribution as estimated by the dash client. Figures \ref{fig:throughput-safe-DSL} - \ref{fig:throughput-safe-FTTP} show the throughput distribution as exposed by the dash client. Refer to the discussion below for more details on how these are different.

The throughput estimate that dash.js uses as a heuristic for which encoding should be requested next is a function of the previous measurements and is subject to an additional so called `'`safety'' factor. This value is different from the direct throughput estimation that the player calculated immediately after receiving each chunk. We call the former advertised throughput and the latter calculated throughput.
...
% \todo{verify if statement below is correct}
% Furthermore, our initial hypothesis that New Congestion Window Validated connections would yield more consistent link estimations is confirmed for the FTTC and FTTP scenarios, but interestingly pure Reno's confidence in the DSL connection is higher.
...
\begin{figure*}
  \centering
  \includegraphics[width=\textwidth]{figures/throughput_precise.pdf}
  \caption{Calculated Throughput}
  \label{fig:throughput-precise}
\end{figure*}
...
\begin{figure*}
  \centering
  \includegraphics[width=\textwidth]{figures/throughput_safe.pdf}
  \caption{Advertised Throughput}
  \label{fig:throughput-safe}
\end{figure*}
