...
In HTTP video streaming playback stability is an important metric contributing to the user's quality of experience. Throughput adaptive algorithms are known to sometimes achieve lower levels of stability. New congestion window validation is a proposal aiming to keep the transport's transfer rate more stable. This paper shows that this transport change also translates to improved playback stability in the application layer. We find that enabling New CWV can reduce the rebuffering events with up to \todo{}\% and limit the representation switches by \todo{}\%. Finally, open a discussion about future transport modifications that could further improve playback stability. 
