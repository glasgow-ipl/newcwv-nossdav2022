...
Firstly, in order to present the transport behaviour more accurately, we collect two distinct but related throughout measurements, used by the client: precise and safe. Both are calculated for each received chunk. The former is simply the received bytes divided by the download time. The latter is a function that the dash.js client uses internally to select the next representation. The function takes the precise measurement, as well as historical data for previously measured bandwidth values on that connection and finally applies a safety factor ($<1.0$) to the result. In figures \ref{fig:throughput-clients-1} - \ref{fig:throughput-clients-5} we present the cumulative distribution function for all obtained bandwidth measurements.
