...
As described in Section~\ref{sec:background}, New CWV alters TCP's \texttt{cwnd} sizing behaviour, allowing for faster recovery after idle periods. Therefore, we expect the client to report more stable throughput estimates. 

  \begin{figure*}
    \centering
    \subfloat[1 Client]{
      \includegraphics[height=.15\paperheight]{figures/Throughput_1_clients.pdf}
      \label{fig:throughput-clients-1}
    }
    \\
    \subfloat[2 Clients]{
      \includegraphics[height=.15\paperheight]{figures/Throughput_2_clients.pdf}
      \label{fig:throughput-clients-2}
    }
    \\
    \subfloat[3 Clients]{
      \includegraphics[height=.15\paperheight]{figures/Throughput_3_clients.pdf}
      \label{fig:throughput-clients-3}
    }
    \\
    \subfloat[5 Clients]{
      \includegraphics[height=.15\paperheight]{figures/Throughput_5_clients.pdf}
      \label{fig:throughput-clients-5}
    }
    \caption{Dash.js Client Throughput measurements \todo{Figure 3: remove FTTP, organize DSL, FTTC as in Fig 4 (landscape)}}
