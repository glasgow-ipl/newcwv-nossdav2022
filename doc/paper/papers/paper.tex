...
A ``standard'' TCP connection (as defined in the Kernel). Would erase all of its state after an idle period longer than the RTO, while a new congestion window validation one would sustain knowledge of its slow start threshold. This results in the former connection re-entering its slow start phase, drastically increasing its sending rate and relying on packet loss to adjust to the connection's current capacity. On the other hand, a new congestion window validation controlled connection would only re-enter slow start up to its previously known slow start threshold and enter congestion avoidance phase after that.

Essentially, this allows a New Congestion Window validated connection to resume operation as normal faster. Furthermore, by not searching for the available capacity after a timeout period, these connections are less aggressive to the middleboxes and account for fewer lost packets caused by overfilling the network buffers. In addition, by preventing the initial CWND spike after a timeout period, New Congestion Window validated connections' average bandwidth converges to this of the link capacity faster. This is critical, given that HTTP video leverages this on-off behaviour often and in our dataset individual video chunks were not bigger than 120 kilobytes for the highest video encoding.

Fast converging estimation mechanisms for the average link capacity is critical in environments such as dash.js. In it one of the adaptation algorithms heavily relies on this method. It uses the estimation from one chunk to decide what encoding should be requested next. In other words, an accurate representation would enable the adaptation algorithm to request the correct encoding more often. Equally, an imprecise calculation could potentially force a wrong encoding to be requested hindering the QoE.


Our hypothesis is that New Congestion Window validated connections would be able to more accurately estimate the link capacity which could potentially also yield higher QoE.

% With video sized chunks, these two transfer patterns would yield different results about the connection's transfer rate. The former one would have lower consistency if that scenario is repeated over time, while the latter will have a more precise confidence interval.

% Since dash.js is performing similar evaluations as the ones described above to gauge the connection's throughput, this lead us to believe that connections that do not use new congestion window validation might show higher variance in estimating the connection's capacity after an idle period. In turn, such a wrong estimation could trick the bit-rate selection algorithm into requesting a too conservative or a too aggressive video chunk. The next chapter summarizes our findings on whether our hypothesis currently holds for DSL, FTTC, and FTTP connections.  

\section{Experimental Setup}

As discussed in section \ref{sec:newcwv-impact} the goal of this paper is to show whether New Congestion Window Validation's improved transport performance translates to a smoother video playout in the application layer. Our hypothesis is that the faster congestion window resumption will enable the client to obtain more precise bandwidth estimations. In turn, when the throughput estimation is the main or only quality selecting heuristic (e.g., the throughput algorithm in dash.js) these more precise estimations could lead to fewer quality switches and higher average bit-rate. Both directly impacting the achieved QoE.

To evaluate our hypothesis we design an experiment using a dumbbell topology. One of the ends acts as a server (nginx 1.18 HTTP/2) and the other one as a client (dash.js 3.1.3 in Firefox 91). The adaptation algorithm is set to throughput as explained in the previous paragraph. The throughput algorithm is currently one of the two adaptive strategies used in dash.js's default selection algorithm. Therefore, evaluating the impact of the throughput algorithm directly carries these findings to the default ``DYNAMIC'' algorithm. 
...
Two distinct simulation instances are run each time: one using TCP Reno but not New Congestion Window Validation. The other one also uses TCP Reno but has New Congestion Window Validation. To eliminate noise, we run each such instance 10 times before averaging and reporting the results.
...
Figures \ref{fig:throughput-precise-DSL} - \ref{fig:throughput-precise-FTTP} show the distribution of the different throughput calculations as estimated by the dash client. Figures \ref{fig:throughput-safe-DSL} - \ref{fig:throughput-safe-FTTP} show the distribution of different throughput calculations as exposed by the dash client. Refer to the discussion below for more details on how these are different.

Figures \ref{fig:avg-bitrate-DSL} - \ref{fig:avg-oscillation-FTTP} show metrics immediately impacting the video QoE. In particular, Figures \ref{fig:avg-bitrate-DSL} - \ref{fig:avg-bitrate-FTTP} show the achieved average bitrate by the video stream over the full playout duration. Finally, Figures \ref{fig:avg-oscillation-DSL} - \ref{fig:avg-oscillation-FTTP} show the bitrate oscillation index for both streams.

In summary, we found that New CWV connections have a lower value for the estimated throughput, compared to Reno connections without newcwv. \todo{More on that when I add the distribution plots}
Additionally, we found that in the tested network scenarios Reno with and without New CWV displayed similar performance in terms of the QoE defining metrics. Therefore, in these cases New CWV makes no difference w.r.t. the QoE or video smoothness.


\todo{Distribution plots for the throughput}

Dash.js exposes one value for the calculated bandwidth but internally works with two. The advertised one is an average based on previous calculations and then further reduced by the so-called ``safety'' factor. The second, internal one is the immediate bandwidth that the client calculates after the video chunk transfer was completed. In Figures \ref{fig:throughput-precise-DSL} to \ref{fig:throughput-safe-FTTP} we report both and call the former advertised and the latter calculated throughput.

In therms of the calculated throughput, it should be noted that the New Congestion Window Validation connections consistently report lower throughput estimations compared to their counterpart. We reason this comes from the fact that New Congestion Window Validated connections are less aggressive and do not send as many bursts over the link capacity as the alternative. In turn these episodes where Reno overshoots account for the higher overall reported link bitrate. Furthermore, our initial hypothesis that New Congestion Window Validated connections would yield more consistent link estimations is confirmed for the FTTC and FTTP scenarios, but interestingly pure Reno's confidence in the DSL connection is higher.

Figures (\ref{fig:avg-bitrate-DSL} to \ref{fig:avg-oscillation-FTTP}) represent video QoE defining metrics. In these, we see that under the examined scenario with the given set of representations and link constraints Reno performs the same regardless of whether New Congestion Window validation is enabled.
...
\begin{figure*}
  ...
  \caption{Average Bitrate Per Connection Type}
  \label{fig:combined}
\end{figure*}
